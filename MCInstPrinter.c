...
static bool testFeatureBits(const MCInst *MI, uint32_t Value) {
  assert(MI);
  switch (MI->csh->arch) {
  default: assert(0 && "Not implemented for current arch.");
  case CS_ARCH_ARM:
    return ARM_getFeatureBits(MI->csh->mode, Value);
  }
}

static bool matchAliasCondition(MCInst *MI,
  ...
  case AliasPatternCond_K_Imm:
    ...
    return MCOperand_isImm(Opnd) && MCOperand_getImm(Opnd) == (int32_t)C->Value;
  ...
  case AliasPatternCond_K_TiedReg:
    ...
    return MCOperand_isReg(Opnd) && MCOperand_getReg(Opnd) == MCOperand_getReg(MCInst_getOperand(MI, C->Value));
  case AliasPatternCond_K_RegClass:
    // Operand must be a register in this class. Value is a register class id.
    return MCOperand_isReg(Opnd) && MCRegisterClass_contains(MCRegisterInfo_getRegClass(MRI, C->Value), MCOperand_getReg(Opnd));
...
const char *matchAliasPatterns(MCInst *MI, const AliasMatchingData *M) {
  ...
  for (const AliasPattern *P = Patterns; P != Patterns + M->OpToPatterns->NumPatterns; ++P) {
