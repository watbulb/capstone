...
#define DECLARE_DecodeT2AddrModeImm7(shift, WriteBack)                         \
  static DecodeStatus CONCAT(DecodeT2AddrModeImm7, CONCAT(shift, WriteBack))(  \
      MCInst * Inst, unsigned Val, uint64_t Address, const void *Decoder);
...
DecodeStatus AddThumbPredicate(MCInst *MI)
{
  ...
  unsigned i;
  for (i = 0; i < NumOps; ++i) {
    if (MCOperandInfo_isPredicate(&OpInfo[i]) || i == MCInst_getNumOperands(MI))
      break;
  }
...
void UpdateThumbVFPPredicate(DecodeStatus S, MCInst *MI)
{
  ...
  for (unsigned i = 0; i < NumOps; ++i) {
    if (MCOperandInfo_isPredicate(&OpInfo[i])) {
      if (CC != ARMCC_AL && !MCOperandInfo_isPredicate(&OpInfo[i]))
	Check(&S, MCDisassembler_SoftFail);
				 ...
				 void *Info)
{
  ...
  if (Result != MCDisassembler_Fail) {
    *Size = 2;
    ...
    if (MCInst_getOpcode(MI) == ARM_t2IT) {
      ...
      ITBlock_setITState(&(MI->csh->ITBlock), (char)Firstcond, (char)Mask);
  ...
  if (fieldFromInstruction_4(Insn32, 24, 4) == 0xF) {
    ...
    Result =
	decodeInstruction_4(DecoderTableNEONData32, MI, NEONDataInsn, Address);
    if (Result != MCDisassembler_Fail) {
      *Size = 4;
      Check(&Result, AddThumbPredicate(MI));
      return Result;
					   ...
					   const void *Decoder)
{
  ...
  if (MCInst_getOpcode(Inst) == ARM_tBcc && Val == 0xE)
    return MCDisassembler_Fail;
  if (Val != ARMCC_AL &&
      !MCInst_isPrediacable(Inst, &ARMInsts[MCInst_getOpcode(Inst)]))
    Check(&S, MCDisassembler_SoftFail);
...
static DecodeStatus DecodeTSBInstruction(MCInst *Inst, unsigned Insn,
					 uint64_t Address, const void *Decoder)
{
  if (MCInst_getOpcode(Inst) != ARM_TSB && MCInst_getOpcode(Inst) != ARM_t2TSB)
    return MCDisassembler_Fail;
					       ...
					       const void *Decoder)
{
  ...
  if (!tryAddingSymbolicOperand(Address, Address + SignExtend32((imm), 26) + 8,
				true, 4, Inst, Decoder))
    MCOperand_CreateImm0(Inst, (SignExtend32((imm), 26)));
...
static DecodeStatus DecodeT2BROperand(MCInst *Inst, unsigned Val,
				      uint64_t Address, const void *Decoder)
{
  if (!tryAddingSymbolicOperand(Address, Address + SignExtend32((Val), 21) + 4,
				true, 4, Inst, Decoder))
...
#define DEFINE_DecodeTAddrModeImm7(shift)                                      \
  ...
  {                                                                            \
    ...
    if (!Check(&S, DecodetGPRRegisterClass(Inst, Rn, Address, Decoder)))       \
      return MCDisassembler_Fail;                                              \
    if (!Check(&S, CONCAT(DecodeT2Imm7, shift)(Inst, imm, Address, Decoder)))  \
      return MCDisassembler_Fail;                                              \
                                                                               \
    return S;                                                                  \
  }
...
{
  if (!tryAddingSymbolicOperand(Address,
				Address + SignExtend32((Val << 1), 9) + 4, true,
				2, Inst, Decoder))
...
static DecodeStatus DecodeVpredNOperand(MCInst *Inst, unsigned RegNo,
					uint64_t Address, const void *Decoder)
{
			 ...
			 const void *Decoder)
{
  ...
  if (!Check(&S, DecodeMQPRRegisterClass(Inst, Qd, Address, Decoder)))
    return MCDisassembler_Fail;
  if (!Check(&S, CONCAT(DecodeMVEPairVectorIndexOperand, 2)(Inst, index,
							    Address, Decoder)))
    return MCDisassembler_Fail;
  if (!Check(&S, CONCAT(DecodeMVEPairVectorIndexOperand, 0)(Inst, index,
							    Address, Decoder)))
    return MCDisassembler_Fail;
...
#define DEFINE_DecodeMVEVCMP(scalar, predicate_decoder)                        \
  ...
  {                                                                            \
    ...
    if (scalar) {                                                              \
      ...
      unsigned Rm = fieldFromInstruction_4(Insn, 0, 4);                        \
      if (!Check(&S,                                                           \
		 DecodeGPRwithZRRegisterClass(Inst, Rm, Address, Decoder)))    \
	return MCDisassembler_Fail;                                            \
    } else {                                                                   \
      fc = fieldFromInstruction_4(Insn, 12, 1) << 2 |                          \
	   fieldFromInstruction_4(Insn, 7, 1) |                                \
